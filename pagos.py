...
from lib import *
from con import get_con, log, engine, check_roles, get_conbuf
...
def loterbo_imprimir(fecha, cobr, idlote):
    ...
    estimado = pgdict(
        con,
        f"select sum(monto) as cuotas from estimados where cobr={cobr} and mes=date_format(curdate(),'%Y%m')")
    cobrado = pgonecolumn(
        con,
        f"select sum(imp+rec) from pagos where cobr={cobr} and date_format(fecha,'%Y-%m')=date_format(curdate(),'%Y-%m')")
    if cobrado is None:
        cobrado = 0
...
def loterbo_reimprimir(fecha, cobr, idlote):
    ...
    estimado = pglistdict(
        con, f"select date_format(pmovto,'%Y-%m') as periodo,sum(cuota) as cuotas from clientes,zonas where clientes.zona=zonas.zona and pmovto>date_sub(curdate(),interval 180 day)  and zonas.zona not like '-%' and asignado={cobr} group by periodo having periodo=date_format(curdate(),'%Y-%m')")[0]
    cobrado = pgonecolumn(
        con,
        f"select sum(imp+rec) from pagos where cobr={cobr} and date_format(fecha,'%Y-%m')=date_format(curdate(),'%Y-%m')")
    if cobrado is None:
...
def loterbo_getlistalotesrbo():
    con = get_con()
    lotesrbo = pglistdict(
        con, f"select id,fecha,cobr,cnt from loterbos order by id desc limit 100")
    con.close()
...
def loterbo_buscanombrecobr(cobr):
    con = get_con()
    nombrecobr = pgonecolumn(
        con, f"select nombre from cobr where id={cobr} and activo=1 and prom=0")
    con.close()
    if len(nombrecobr) > 0:
...
def loterbo_getidcobradores():
    con = get_con()
    idcobradores = pglist(
        con, "select id from cobr where activo=1 and prom=0 and id>500")
...
def pagos_listado(fechapago, cobrador):
    ...
    listado = pglistdict(
        con,
        f"select pagos.id as id, rbo, fecha, idvta,imp as imp, rec as rec, \
            (imp+rec) as total, nombre, concat(calle,' ',num) as direccion, \
                zona,deuda as deuda from pagos, clientes where \
                    clientes.id=pagos.idcliente and fecha='{fechapago}' \
                        and pagos.cobr={cobrador} order by id desc")
    planilla = pglistdict(
        con,
        f"select * from planillas where fecha='{fechapago}' and \
            idcobr={cobrador}")
    transferencia = pgonecolumn(con, f"select imp from caja where fecha=\
                                '{fechapago}' and codigo={cobrador} and \
                                cuenta='transferencia de cobradores'")
    con.close()
...
def pagos_pasarplanilla():
    ...
    if re.match(solo_digitos, d_dato['transferencia']):
        ...
        transferencia = 0
    idplanilla = pgonecolumn(con, f"select id from planillas where fecha = \
                            '{d_dato['fecha']}' and idcobr={d_dato['idcobr']}")
    idasiento = pgonecolumn(con, f"select id from caja where fecha=\
                        '{d_dato['fecha']}' and codigo={d_dato['idcobr']} \
                        and cuenta='transferencia de cobradores'")
    idasiento1 = pgonecolumn(con, f"select id from caja where fecha=\
                        '{d_dato['fecha']}' and codigo={d_dato['idcobr']} \
                        and cuenta='naranjax ingreso cobradores'")
    if (d_dato['viatico'] == ""):
        d_dato['viatico'] = 0
    ins = f"insert into planillas(fecha, idcobr, idlote,cobrado, comision, \
        viatico, cntrbos) values('{d_dato['fecha']}','{d_dato['idcobr']}',0,\
            {d_dato['cobrado']},{d_dato['comision']},{d_dato['viatico']},\
                {d_dato['cntrbos']})"
    upd = f"update planillas set fecha='{d_dato['fecha']}', \
        idcobr='{d_dato['idcobr']}',idlote=0,cobrado={d_dato['cobrado']},\
            comision={d_dato['comision']},viatico={d_dato['viatico']},\
                cntrbos={d_dato['cntrbos']} where id={idplanilla}"
    updpagos = f"update pagos set rendido=1 where cobr={d_dato['idcobr']} and \
    rendido=0"
    insasiento = f"insert into caja(fecha,cuenta,imp,codigo,comentario,conciliado) \
        values ('{d_dato['fecha']}','transferencia de cobradores',\
            {(-1)*transferencia},{d_dato['idcobr']},\
            'NARANJA X CONFIRMADO',1),\
            ('{d_dato['fecha']}', 'naranjax ingreso cobradores',\
                   {transferencia}, {d_dato['idcobr']},\
                   'NARANJA X CONFIRMADO',1)"
    updasiento = f"update caja set fecha='{d_dato['fecha']}', \
        imp='{(-1)*transferencia}', codigo='{d_dato['idcobr']}' where \
        id={idasiento}"
    updasiento1 = f"update caja set fecha='{d_dato['fecha']}', \
        imp='{transferencia}', codigo='{d_dato['idcobr']}' where \
        id={idasiento1}"
    delasiento = f"delete from caja where id={idasiento}"
    delasiento1 = f"delete from caja where id={idasiento1}"
    try:
        if idplanilla == "":
            pgexec(con,ins)
            log(ins)
            ...
            pgexec(con,upd)
            log(upd)
        if idasiento == "" and transferencia>0:
            pgexec(con,insasiento)
            log(insasiento)
        elif idasiento != "" and transferencia>0:
            pgexec(con,updasiento)
            pgexec(con,updasiento1)
            log(updasiento)
            log(updasiento1)
        elif idasiento != "" and transferencia==0:
            pgexec(con,delasiento)
            pgexec(con,delasiento1)
            log(delasiento)
            log(delasiento1)
        pgexec(con, updpagos)
        log(updpagos)
...
def pagos_getplanillascobr():
    con = get_con()
    planillascobr = pglistdict(
        con,
        f"select fecha,idcobr,cobrado,comision,viatico,cntrbos from planillas order by fecha desc limit 100")
    con.close()
...
def pagos_obtenerrbo(id):
    ...
    reg = pglistdict(
        con,
        f"select fecha, idvta, imp, rec, rbo, cobr, id, (select nombre from clientes where clientes.id=pagos.idcliente) as nombre from pagos where id={id}")
    con.close()
...
def pagos_obtenernuevonombre(idvta):
    con = get_con()
    idcliente = pgonecolumn(
        con, f"select idcliente from ventas where id={idvta}")
    nombre = pgonecolumn(
        con, f"select nombre from clientes where id={idcliente}")
...
def pagos_obtenerregrbo(buscar):
    ...
    try:
        reg = pglistdict(
            con,
            f"select pagos.id as id, fecha, idvta, imp, rec, rbo, cobr, idcliente, nombre from pagos,clientes where clientes.id=pagos.idcliente and  rbo={buscar}")
...
def pagos_guardaredicionrbo():
    ...
    d = json.loads(request.data.decode("UTF-8"))
    idcliente = pgonecolumn(
        con, f"select idcliente from ventas where id={d['idvta']}")
...
def pagos_cobrostotales():
    ...
    tbl = pd.pivot_table(
        df, values=['cuota'],
        index='cobr', columns='fp', aggfunc='sum').sort_index(
        axis=1, level='fp', ascending=False)
    tbl1 = pd.pivot_table(
        df1, values=['cuota'],
        index=['asignado', 'zona'],
        columns='fp', aggfunc='sum').sort_index(
        axis=1, level='fp', ascending=False)
    tbl = tbl.fillna("")
    tbl1 = tbl1.fillna("")
    ...
    return render_template(
        "pagos/totales.html", tbl=tbl, tbl1=tbl1, index=index)
...
def pagos_estimados():
    ...
    tbl = pd.pivot_table(
        df, values=['cuota'],
        index='asignado', columns='pmovto', aggfunc='sum').sort_index(
        axis=1, level='pmovto', ascending=False)
    tbl1 = pd.pivot_table(
        df1, values=['cuota'],
        index=['asignado', 'zona'],
        columns='pmovto', aggfunc='sum').sort_index(
        axis=1, level='pmovto', ascending=False)
    tbl = tbl.fillna("")
    ...
    return render_template(
        "pagos/estimados.html", tbl=tbl, tbl1=tbl1, index=index)
...
def pagos_estimadosmes():
    ...
    tbl = pd.pivot_table(
        df, values=['cuota'],
        index='asignado', columns='pmovto', aggfunc='sum')
    tbl = tbl.fillna("")
    tbl1 = pd.pivot_table(
        df, values=['cuota'],
        index='zona', columns='pmovto', aggfunc='sum')
    tbl1 = tbl1.fillna("")
    # index = tbl.columns.values.tolist()
    # aca entregamos el valor de las columnas como indice
    tbl = tbl.to_json()
...
def pagos_comisiones():
    ...
    tbl = pd.pivot_table(
        df, values=['comision'],
        index='fecha', columns='cobr', aggfunc='sum').sort_index(
        axis=0, level='fecha', ascending=False)
    tbl = tbl.fillna("")
    ...
    return render_template(
        "pagos/comisiones.html", tbl=tbl, c750=c750, c796=c796, c800=c800,
        c802=c802, c815=c815, index=index)
...
def pivot_pagos_cobr():
    ...
    df = pd.DataFrame(dat)
    tbl = pd.pivot_table(
        df, values=['comision', 'cobranza'],
        index='fecha', columns='cobr', aggfunc='sum')
    tbl = tbl.fillna("")
...
def pivot_retiros():
    ...
    dat = pd.read_sql_query(sql, engine)
    df = pd.DataFrame(dat)
    tbl = pd.pivot_table(df, values='imp', index='fecha', columns='cuenta',
...
def pivot_retiros_excel():
    ...
    dat = pd.read_sql_query(sql, engine)
    df = pd.DataFrame(dat)
    tbl = pd.pivot_table(df, values='imp', index='fecha',
                         columns='cuenta', aggfunc='sum')
    tbl = tbl.fillna("")
...
def pagos_loadaltas():
    con = get_con()
    listaltas = pglistdict(
        con,
        f"select id,nombre,dni,sex, calle,num,deuda,ultpago,subirseven from clientes where subirseven=1 and alta is null order by deuda")
    con.close()
...
def pagos_sevenaltas():
    con = get_con()
    sevenaltas = pglisttuples(
        con,
        f"select 2210,dni,dni,3,sex,'','',nombre,concat(calle,' ',num),5000,barrio,'Cordoba','Cordoba','M', date_format(current_date(),'%Y-%m-%d'),0,'01','',wapp from clientes where subirseven=1 and alta is null")
    con.close()
...
def pagos_marcarsubidos():
    ...
    try:
        ...
        for dni in listadni:
            idcliente = pgonecolumn(
                con, f"select id from clientes where dni={dni}")
...
def pagos_loadbajas():
    con = get_con()
    listbajas = pglistdict(
        con,
        f"select dni,nombre,dni, 'Cancelado' as canc, ultpago from clientes where sev=1 and deuda=0")
    con.close()
...
def pagos_getcobroscobrador(cnt):
    ...
    (10,816,835,820)")
    return jsonify(
        listacobros=listacobros, listamesesestimados=listamesesestimados,
        cobr=listacobr)
...
def pagos_getnickcobrador(cobr):
    ...
    nick, wapp = pgtuple(
        con, f"select nick,telefono from cobr where id={cobr}")
